...
// ResponseWriterWrapper wraps http.ResponseWriter to catch the status code
// and byte count. It implements the WriteHeader() and Write() interfaces
...
// CombinedLogger and CommonLogger are middlewares that log all http
// requests and response in Apache CommonLog and CombinedLog formats.
...
func CombinedLogger(out io.Writer) Middleware {
	return func(next http.Handler) http.Handler {
		fn := func(w http.ResponseWriter, r *http.Request) {
			...
			next.ServeHTTP(&ww, r)
			t2 := time.Now()
			writeCombinedLog(out, r, t1, ww.StatusCode, ww.Bytes, t2.Sub(t1))
		}
...
func CommonLogger(out io.Writer) Middleware {
	return func(next http.Handler) http.Handler {
		fn := func(w http.ResponseWriter, r *http.Request) {
			t1 := time.Now()
			ww := ResponseWriterWrapper{ResponseWriter: w}
			next.ServeHTTP(&ww, r)
			t2 := time.Now()
			writeCommonLog(out, r, t1, ww.StatusCode, ww.Bytes, t2.Sub(t1))
		}
		return http.HandlerFunc(fn)
	}
}
